...
export default function TeachSomeone() {
  ...
  return (
    <section className="flex flex-col md:flex-row md:gap-20 items-center justify-between px-4 md:px-28 pt-10 md:pt-20 pb-8 md:pb-20 bg-white">
      {/* Left Side */}
      <div className="w-full md:w-2/7 md:text-left">
        <h2 className="font-noto text-xl md:text-2xl font-bold text-primary md:w-[100%] w-[90%] md:my-2 mb-4">
        ...
        {teachingPoints.map((point, index) => (
          <div key={index} className="flex items-start gap-4 pt-3 w-full">
            {/* Number Circle */}
            <div className="w-16 h-6 rounded-full bg-gray-300  flex items-center justify-center text-black font-bold">
              {point.number}
            </div>
            
            {/* Title & Description */}
            <div>
              <h3 className="font-noto">{point.title}</h3>
              <p className="font-roboto text-sm text-gray-800 mt-3" dangerouslySetInnerHTML={{ __html: point.description }} />
            </div>
          </div>
        ))}
      </div>
